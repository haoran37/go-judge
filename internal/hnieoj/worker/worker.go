// Package worker 实现有界槽位任务执行：任务来自 WSS 任务通道（Claim），
...
// Claimer 是 WSS 任务通道：每次至多返回一个任务；无任务时阻塞直到 ctx 取消。
type Claimer interface {
	Claim(ctx context.Context) (*model.Assignment, error)
}
...
// Handler 处理一个任务；ctx 在租约丢失、TASK_CANCEL 或排空超时时被取消。
...
type Options struct {
	...
	// Registry 记录在途 attempt 的租约与取消函数；为 nil 时不做注册。
	Registry *tasks.Registry
	// Now 提供服务端校正后的时间；为 nil 时使用 time.Now。
	Now func() time.Time
}
...
func (p *Pool) Run(ctx context.Context) error {
	...
	<-ctx.Done()
	// 停止新领取；WSS claim 阻塞会随 claimCtx 取消立即返回。
...
func (p *Pool) runTask(runCtx context.Context, claim *model.Assignment) {
	task := claim.Task
	task.AttemptID = claim.AttemptID
	lease := &leaseState{untilMillis: claim.LeaseUntil, now: p.now}
	// 若 RESUME 已更新过租约/epoch，执行器必须采用 registry 的最新值。
	if p.registry != nil {
		if snap, ok := p.registry.Get(task.SubmissionID, task.JudgeTaskID, claim.AttemptID); ok {
			if snap.LeaseUntil > 0 {
				lease.update(snap.LeaseUntil)
			}
			p.registry.SetLeaseObserver(task.SubmissionID, task.JudgeTaskID, claim.AttemptID, lease.update)
		}
	}
	...
	taskCtx, taskCancel := context.WithCancel(base)
	defer taskCancel()

	// TASK_CANCEL / epoch fence 已取消：不执行，直接清理注册。
	if claim.Ctx != nil && claim.Ctx.Err() != nil {
		p.complete(claim)
		return
	}
...
func (p *Pool) complete(claim *model.Assignment) {
	if p.registry == nil || claim == nil {
		return
	}
	p.registry.Complete(claim.Task.SubmissionID, claim.Task.JudgeTaskID, claim.AttemptID)
}

func (p *Pool) renewLoop(taskCtx context.Context, cancel context.CancelFunc, claim *model.Assignment, lease *leaseState) {
...
func (p *Pool) nextRenewDelay(lease *leaseState, period time.Duration) time.Duration {
	...
	if deadline := lease.deadline(); !deadline.IsZero() {
		...
		limit := deadline.Sub(p.now()) - safety
		if limit < delay {
			delay = limit
		}
	}
...
func (p *Pool) renewWithRetry(taskCtx context.Context, claim *model.Assignment, lease *leaseState) bool {
	for {
		if taskCtx.Err() != nil {
			return false
		}
		...
		if err == nil {
			// 迟到的成功响应不得复活已经过期的租约。
			if until <= 0 || !p.now().Before(deadline) {
			...
			lease.update(until)
			if p.registry != nil {
				p.registry.UpdateLease(claim.Task.SubmissionID, claim.Task.JudgeTaskID, claim.AttemptID, until)
			}
		...
		remaining := lease.deadline().Sub(p.now())
		if remaining <= 0 {
...
func (l *leaseState) clock() time.Time {
	if l.now != nil {
		return l.now()
	}
	return time.Now()
...
func (l *leaseState) expired() bool {
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.untilMillis <= 0 {
		return true
	}
	return !l.clock().Before(time.UnixMilli(l.untilMillis))
}
